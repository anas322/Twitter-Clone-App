    ...
    <div class="hidden pt-2 meduim:block small:col-span-6 meduim:col-span-6">
        <div class="border-x border-gray-200 dark:border-white/20 relative h-screen">
            ...
            <div v-if="props.messages.length == 0">
                <div class="flex flex-col gap-y-2 max-w-sm mx-auto pt-96">
                    <span class="text-4xl font-bold dark:text-white">Select a message</span>
                    <span class="text-sm text-gray-500"
                        >Choose from your existing conversations, start a new one, or just keep swimming.</span
                    >
                </div>
            </div>
            ...
            <div v-else class="relative h-screen flex flex-col justify-between">
                ...
                <div class="z-10 absolute left-0 right-0 top-0 backdrop-blur-md bg-[#ffffff78] dark:bg-[#ffffff05]">
                    <div class="p-2 flex items-center gap-x-6">
                        <div class="flex items-center gap-x-2">
                            <div>
                                <NuxtLink :to="`/profile/${props.recipient.username}`">
                                    <img
                                        v-if="props.recipient.profile.avatar"
                                        :src="props.recipient.profile.avatar"
                                        :alt="props.recipient.name"
                                        class="w-7 h-7 rounded-full object-cover"
                                    />
                                    <UIDefaultAvatar v-else class="w-7 h-7 rounded-full overflow-hidden bg-gray-600" />
                                </NuxtLink>
                            </div>
                            <span class="font-bold dark:text-white">{{ props.recipient.name }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div ref="messageContainer" class="overflow-y-auto grow">
                    <div class="p-3 min-h-full flex flex-col justify-end gap-y-4">
                        <template v-for="mssg in props.messages" :key="`message-${mssg.id}`">
                            <div
                                class="grid gap-y-1"
...
const sendMessage = () => {
    if (message) {
        emits("onSendMessage", message.value, props.recipient.id);
        message.value = "";
    }
};
</script>
